...
이벤트성 트래픽이 급증하면서 상황이 달라졌다. Redis 세션 서버의 CPU와 커넥션 수가 예상치 못한 수준으로 치솟기 시작했다. Redis MONITOR로 확인하자 `/api/event/**` 경로로 들어오는 모든 요청마다 세션 키 조회 명령이 발생하고 있었다. `permitAll()` 설정이 있음에도 불구하고.
